...
export default function ExpenseDayGroup({
  ...
  const expenseDayGroupCalendarDate = new Date(
    dayExpenseGroup.calendarDate,
  ).toLocaleDateString();
  ...
  return (
    <div className="my-4">
      ...
      >
        ...
        <div
          className={`dotted-line ${publicUserData.darkModeEnabled && "dotted-line-dark"}`}
        ></div>
        <p className="text-3xl font-bold">
          {formatDollarAmountStatic(dayTotal, publicUserData.currency)}
        </p>
      </div>
